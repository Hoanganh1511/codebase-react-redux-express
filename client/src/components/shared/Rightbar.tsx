...
import { getPublicUsersAction, followUserAndFetchData } from '../../redux/actions/userActions'
...
const Rightbar = () => {
  ...
  const currentUser = useSelector((state: ReturnType<typeof rootReducer>) => state.auth?.userData)

  const recommendedUsers = useSelector((state: ReturnType<typeof rootReducer>) => state.user?.publicUsers)
  ...
  const notJoinedCommunities = useSelector(
    (state: ReturnType<typeof rootReducer>) => state.community?.notJoinedCommunities
  )
  ...
  const toggleJoinModal = useCallback((communityId: string, visible: boolean) => {
  ...
  return (
    <div className='hidden rightbar overflow-auto'>
      {currentLocation !== '/communities' && (
        <div>
          <div className='flex items-end justify-between mb-4'>
            ...
            {remainingCount > 0 && (
              <Link className='flex relative items-center text-sm font-medium text-primary mr-4' to='/communities'>
                See all
          ...
          {notJoinedCommunitiesFetched && visibleCommunities.length === 0 && (
            <div className='text-center italic text-gray-400'>No communities to join. Check back later.</div>
          )}
          <ul className='flex flex-col gap-3'>
            {visibleCommunities?.map((community: any) => (
              <li key={community._id} className='flex items-center justify-between bg-white px-2 py-1 rounded-md'>
      ...
      {publicUsersFetched && recommendedUsers?.length === 0 && (
        <div className='text-center italic text-gray-400'>No users to follow. Check back later</div>
      )}
      <ul className='flex flex-col gap-3'>
          ...
          recommendedUsers.map((user: any) => (
            ...
            >
              <div className='flex justify-content-between items-center gap-1'>
                <img className='h-7 w-7 rounded-full flex-shrink-0 object-cover' src={user.avatar} alt={user.name} />
                <div>
                  <Link to={`/user/${user._id}`} className='font-medium text-base line-clamp-1'>
                    {user.name}
                  </Link>
