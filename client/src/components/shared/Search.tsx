...
const Search = () => {
  ...
  const [joinedCommunity, setJoinedCommunity] = useState<any>(null)
    ...
    <div className='relative'>
      <div className=''>
        <input
          ...
          className='h-10 py-1 bg-white border w-full md:w-[660px] rounded-full text-sm shadow-sm focus:outline-none focus:shadow-outline focus:border-blue-500 transition duration-300 pl-3 pr-10'
          aria-label='Search'
          autoComplete='off'
        />
        {inputValue !== '' && (
      ...
      {inputValue !== '' && (
        <div
          ...
          className='absolute start-0 md:start-auto w-screen top-12 md:w-[660px]  bg-white  rounded-md shadow-md'
        >
          {loading && (
      ...
      {users.length > 0 && (
        <ul className='absolute w-full left-0 top-full bg-white z-30 border rounded-md'>
          {users.map((user: any) => (
      ...
      {community && (
        <div className='border-b py-2 px-4'>
          <div className='flex items-center'>
            <div className='flex-shrink-0'>
              <img src={community.banner} alt={community.banner} className='h-8 w-8 rounded-full' />
            </div>
              ...
              {!community.isMember && (
                <div>
                  <JoinModal
                    ...
                    community={community}
                  />
                  <button className='bg-primary px-2 py-1 text-white  text-sm rounded-md' onClick={() => toggleModal()}>
                    Join
                  </button>
                </div>
              )}
      ...
      {joinedCommunity && (
        ...
        >
          <div className='flex items-center'>
            <div className='flex-shrink-0'>
              <img src={joinedCommunity.banner} alt={joinedCommunity.name} className='h-8 w-8 rounded-full' />
            </div>
            <div className='ml-3'>
              <p className='font-semibold text-md text-primary'>{joinedCommunity.name}</p>
              <p className='text-sm text-gray-600 line-clamp-2'>{joinedCommunity.description}</p>
            </div>
          </div>
        </div>
      )}
