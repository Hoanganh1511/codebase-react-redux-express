...
const CommunityManagement = () => {
  ...
  const communities = useSelector((state: ReturnType<typeof rootReducer>) => state.admin?.communities)
  const moderators = useSelector((state: ReturnType<typeof rootReducer>) => state.admin?.moderators)
  const community = useSelector((state: ReturnType<typeof rootReducer>) => state.admin?.community)
  ...
  return (
    <div className='flex gap-2 h-[85vh] w-full mt-3 border rounded-md'>
      ...
      <div className='flex flex-col w-full bg-white shadow-inner rounded-md border-r'>
        <h1 className='text-lg font-bold p-4 text-center border-b-2'>Communities</h1>
        <div className='flex flex-col overflow-y-auto'>
          {communities.map((community) => (
            ...
            >
              <img src={community.banner} alt={community.name} className='w-10 h-10 rounded-full mr-2 md:mr-4' />
      ...
      <div className='flex flex-col w-full bg-white rounded-md px-5 py-5 border-l'>
        {isChangingCommunity ? (
          ...
          <>
            <h1 className='font-bold text-lg border-b border-black pb-1 mb-2'>{selectedCommunityData.name}</h1>

            {isUpdating && <div className='bg-green-100 text-green-800 p-2 mb-4 rounded'>Updating...</div>}
            <span className='text-sm'>Total Moderators: {selectedCommunityData.moderatorCount}</span>
            <span className='text-sm'>Total Members: {selectedCommunityData.memberCount}</span>

            <div className='flex flex-col md:flex-row gap-5'>
              {/* Moderators list */}
              <div className='flex flex-col gap-2 w-full md:w-1/2'>
                <h2 className='font-medium mb-2'>Moderators</h2>
                {selectedCommunityData.moderators?.length === 0 && <span>No moderators</span>}
                ...
                <div className='flex flex-col gap-2 md:flex-row'>
                  ...
                  <button
                    disabled={
                      !newModerator ||
                      isUpdating ||
                      selectedCommunityData.moderators?.find((moderator) => moderator._id === newModerator)
                    }
          ...
          <div className='flex flex-col items-center justify-center h-full'>
            <span className='font-medium text-gray-400'>Select a community to view details</span>
          </div>
        )}
      </div>
    </div>
