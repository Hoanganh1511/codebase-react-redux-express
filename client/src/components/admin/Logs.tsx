import { deleteLogsAction, getLogsAction } from '@/redux/actions/adminActions'
...
const Logs = () => {
  ...
  const logs = useSelector((state: ReturnType<typeof rootReducer>) => state.admin?.logs)
    ...
    <div className={'bg-white flex flex-col items-center justify-center mt-3 rounded-md'}>
      <div className='p-4 shadow-md rounded-relative xl:min-w-[1200px] lg:min-w-[1000px] md:min-w-[800px]'>
        ...
        <div className='flex justify-between items-center mb-4  border-b border-gray-200 pb-2'>
          <div className='text-sm italic text-gray-600'>{`Showing ${logs.length}`}</div>
          <div className='flex items-center space-x-2'>
            ...
            >
              {clearing ? <ButtonLoadingSpinner loadingText='Clearing...' /> : 'Clear logs'}
            </button>
          </div>
        </div>
              ...
              <div className='h-[430px] relative overflow-auto'>
                <div className='w-full rounded'>
                  <div className='grid grid-cols-5 gap-5 items-center border-b py-2 font-semibold text-gray-800'>
                    <p className='text-center'>Timestamps</p>
                    <p>Message</p>
                    <p>Email Used</p>
                    <p>Level</p>
                    <p>Context Data</p>
                  ...
                  {logs.map((log) => (
                    ...
                    >
                      <span className='flex-col justify-center  items-center text-center font-mono'>
                        <p className=''>{log.relativeTimestamp}</p>
                        <p className='text-xs'>{log.formattedTimestamps}</p>
                      </span>
                      <td
                        className={`${
                          log.level === 'info'
