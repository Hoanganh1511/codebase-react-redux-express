...
const SignUp = () => {
  ...
  const handleAvatarChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.type !== 'image/jpeg' && file.type !== 'image/png' && file.type !== 'image/jpg') {
      setAvatar(null)
      setAvatarError('Please upload a valid image file (jpeg,png,jpg)')
      ...
      <div className='container mx-auto flex min-h-screen items-center justify-center px-6'>
        <form className='w-full max-w-md' onSubmit={handleSubmit}>
          ...
          <div className='mt-6 flex items-center justify-center'>
            <Link to='/signin' className='w-1/3 border-b border-gray-400 pb-4 text-center font-medium text-gray-800'>
              Sign In
            </Link>
            <Link to='/signup' className='w-1/3 border-b-2 border-blue-500 pb-4 text-center font-medium text-gray-800'>
              Sign In
            </Link>
          </div>
          ...
          <div className='mt-6'>
            <button
              disabled={loading}
              type='submit'
              className={`w-full transform rounded-lg bg-blue-500 px-6 py-3 text-center text-sm font-medium tracking-wide text-white transition-colors duration-300 hover:bg-blue-700 focus:outline-none focus:ring focus:ring-blue-300 focus:ring-opacity-50 ${
                loading ? 'cursor-not-allowed opacity-50' : ''
              }`}
            >
              {loading ? <ButtonLoadingSpinner loadingText={loadingText} /> : <span>Sign Up</span>}
            </button>

            <div onClick={() => setIsModalOpen(true)} className='mt-6'>
              {isConsentGiven && !isModerator ? (
                <p className='mt-2 cursor-pointer rounded-lg border border-green px-4 py-3 text-center text-sm font-semibold text-green-600'>
