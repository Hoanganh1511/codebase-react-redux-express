...
const SignIn = () => {
  ...
  const handleSubmit = async (e: React.MouseEvent<HTMLElement>) => {
    ...
    const timeout = setTimeout(() => {
      setLoadingText('This is taking longer than usual. Please wait while backend services are  getting started')
    }, 5000)
  ...
  const signInError = useSelector((state: ReturnType<typeof rootReducer>) => state.auth?.signInError)
    ...
    <section className='bg-white'>
      <div className='container mx-auto flex min-h-screen flex-col items-center justify-center px-6'>
        <form className='w-full max-w-md'>
          ...
          {signInError && (
            ...
            >
              ...
              <button className='font-bold text-red-700' onClick={handleClearMessage}>
          ...
          <div className='mt-6 flex items-center justify-center'>
            ...
            <Link
              to={'/signup'}
              className='w-1/3 border-b-2 border-gray-400 pb-4 text-center font-medium text-gray-500'
            >
              Sign Up
            </Link>
          </div>
        ...
        <span className='flex items-center justify-center py-4 text-sm text-gray-600'>
          ...
          <Link to='/admin' className='ml-8 flex items-center hover:text-blue-500'>
